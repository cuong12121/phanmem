        	...
        	if(!empty($result)){
                ...
                for ($i=0; $i < count($file_pdf_rep) ; $i++) { 
                    ...
                    $link_pdf_href = '<a href="'.$link_pdf.'" target="blank">'.basename($link_pdf).'</a>';
                     array_push($file_pdf_rep1, empty(checkrepair($link_pdf))?$link_pdf_href.'<span style="color:red">(File  pdf gốc đã bị sửa đổi hoặc mã sku không có)</span>':$link_pdf_href);
